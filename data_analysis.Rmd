---
title: "Art Data Analysis"
author: "Jens Laufer"
date: "17 2 2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = F, message = F, warning = T)
```
```{r}
library(tidyverse)
library(glue)
library(mongolite)
library(jsonlite)
library(ggthemes)
library(bbplot)
library(scales)
library(ggrepel)
library(ggcorrplot)
library(forcats)
```

```{r}
con <- mongo("pieces", url="mongodb://localhost/pieces")
```



```{r}
data <- con$find("{}") %>%
  jsonlite::flatten() %>%
  as_tibble() %>%
  filter(year !=  0) %>%
  separate(size, c("width", "height"), sep = " X ") %>%
  mutate(
    width = as.numeric(width)*2.54,
    height = as.numeric(height)*2.54,
    width_unit = "cm",
    height_unit = "cm",
    run = as.numeric(run),
    area = width * height,
    collections = as.numeric(collections),
    sold = as.numeric(sold)
  ) %>% 
  mutate(released=as.Date(as.POSIXct(as.numeric(released), origin="1970-01-01"))) %>% 
  separate(artist, c("artist_surname", "artist_firstname"), sep=", ") %>% 
  mutate(artist_firstname=replace_na(artist_firstname,"")) %>%
  unite(artist, artist_firstname, artist_surname, sep=" ") %>% 
  mutate(artist=str_trim(artist))
```



```{r}
data %>% write_csv("art_pieces.csv")
```



```{r}
data %>% 
  glimpse()
```
## Missing Values


```{r}
missingsValues <- data %>% select(-contains("_unit")) %>%
  mutate(row=row_number()) %>% 
  gather(-row, key="feature", value = "value") %>% 
  mutate(isNa=is.na(value)) %>% 
  group_by(feature) %>% 
  mutate(total=n()) %>% 
  ungroup() %>% 
  select(-value) %>% 
  group_by(feature,isNa,total) %>% 
  summarise(n=n()) %>%
  mutate(ratio=n/total*100) %>% 
  select(feature, isNa, ratio)

labels <- missingsValues %>% 
  filter(isNa==F) %>% 
  arrange(ratio) %>% 
  pull(feature)


 missingsValues %>% 
  ggplot(aes(x = feature, y=ratio, fill=isNa, label=paste0(round(ratio, 2),"%")))+
  geom_bar(stat = 'identity') +
  geom_text(position=position_stack(vjust=0.5)) +
  scale_x_discrete(limits=labels) +
  coord_flip()
```


```{r}
data %>% 
  mutate(row=row_number()) %>% 
  gather(-row, key="feature", value = "value") %>% 
  mutate(isNa=is.na(value)) %>% 
  group_by(feature) %>% 
  mutate(total=n()) %>% 
  ungroup() %>% 
  select(-value) %>%
  group_by(feature) %>% 
  slice(228000) %>% 
  ggplot(aes(x=feature, y=row, fill=isNa))+
  geom_raster()+
  coord_flip()
```


## Univariate Analysis

### Continous Variables
```{r fig.height=10, fig.width=20}
data %>% 
  select(-year, -artist_id, -id) %>% 
  select_if(is.numeric) %>% 
  gather(key = "feature", value = "value") %>% 
  filter(!is.na(value)) %>% 
  ggplot(aes(x = value, y = 0)) +
  geom_boxplot() +
  scale_x_continuous(trans="log10", labels = comma) +
  facet_wrap(~feature,scales = "free")
```

## Categorical Variables

```{r fig.height=10, fig.width=20}
data %>%
  select_if(negate(is.numeric)) %>%
  select(-released,-artwork,-artist,-paper) %>%
  select(-contains("_unit")) %>%
  gather() %>%
  group_by(key, value) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  ggplot(aes(x = reorder(value, n), y = n)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(trans = "log10", labels = comma) +
  coord_flip() +
  facet_wrap( ~ key, scales = "free")
```





```{r fig.height=10, fig.width=20}
data %>% 
  mutate(year_bucket=cut(year, 25, dig.lab=4)) %>% 
  group_by(year_bucket) %>%
  summarise(num=n()) %>% 
  ggplot(aes(x = reorder(year_bucket,num), y=num)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = comma, trans = "log10") +
  coord_flip() 
```

```{r}
data %>% 
  group_by(artist) %>%
  summarise(num=n()) %>%
  mutate(label=if_else(num>500, artist, "")) %>% 
  ggplot(aes(x = "", y = num)) +
  geom_boxplot() +
  geom_label_repel(aes(label=label))+
  scale_y_continuous(trans="log10")+
  coord_flip()
```




## Bivariate Analysis

```{r}
data %>% 
  select(-year, -artist_id, -id) %>% 
  select_if(is.numeric) %>% 
  drop_na() %>% 
  cor() %>% 
  ggcorrplot(method = "circle")
```
```{r fig.height=10, fig.width=20}
numericCols <- data %>% 
  select(-year, -artist_id, -id) %>% 
  select_if(is.numeric) %>% 
  colnames()

data %>% 
  select(-year, -artist_id, -id) %>% 
  select_if(is.numeric) %>% 
  gather(-average_price, key="feature", value="value") %>% 
  drop_na() %>% 
  ggplot(aes(x = average_price, y=value)) +
  geom_point(alpha=.2) +
  geom_smooth()+
  scale_y_continuous(trans = "log10") +
  scale_x_continuous(trans = "log10") +
  facet_wrap(~feature, scales = "free")
  
```
```{r}
data %>% 
  arrange(-area) %>% 
  head(10)
```





```{r}
plotQuantQual <-
  function(data,
           var1,
           var2,
           .trans = "log10") {
    data %>%
      drop_na(!!sym(var1), !!sym(var2)) %>%
      ggplot(aes(
        x = !!sym(var2),
        y = !!sym(var1)
      )) +
      geom_jitter(alpha = .3) +
      scale_x_discrete(
        limits = data %>%
          group_by(!!sym(var2)) %>%
          summarise(median = median(!!sym(var1), na.rm = T)) %>%
          arrange(-median) %>%
          pull(!!sym(var2))
      ) +
      scale_y_continuous(trans = .trans, labels = comma) +
      geom_boxplot(alpha = .7)
  }

plotQuantQuant <-
  function(data,
           var1,
           var2,
           .transVar1 = "identity", .transVar2= "identity") {
    data %>%
      drop_na(!!sym(var1), !!sym(var2)) %>%
      ggplot(aes(
        x = !!sym(var2),
        y = !!sym(var1)
      )) +
      geom_point(alpha = .3) +
      scale_y_continuous(trans = .transVar2, labels = comma) +
      scale_x_continuous(trans = .transVar1, labels = comma, guide = guide_axis(angle = 45))
  }


plotQualQual <-
  function(data,
           var1,
           var2,
           .trans = "identity") {
    data %>%
      drop_na(!!sym(var1), !!sym(var2)) %>%
      group_by(!!sym(var1), !!sym(var2)) %>%
      summarise(num=n()) %>% 
      ungroup() %>% 
      ggplot(aes(
        x = !!sym(var2),
        y = !!sym(var1),
        fill = num
      )) +
      scale_fill_continuous(trans=.trans) +
      scale_x_discrete(guide = guide_axis(angle = 45)) +
      geom_raster() 
  }
```

```{r fig.height=10, fig.width=20}
list(quantitatives=c("average_price","average_price","average_price"),qualitatives=c("technique","class","status")) %>% 
pmap(~data %>% 
  plotQuantQual(..1, ..2))

```

```{r}
list(quantitatives=c("class"),qualitatives=c("technique")) %>% 
pmap(~data %>% 
  plotQualQual(..1, ..2), .trans="log10")
```



